...
def f(sample):
    ...
    x_f1 = sample[2]*(delta_x) + x_min
    y_f1 = sample[3]*(delta_y) + y_min
    x_f2 = sample[4]*(delta_x) + x_min
    y_f2 = sample[5]*(delta_y) + y_min
    t = sample[6]*t_f
...
def pde_fn(model, sample):
    ...
    ddtau = _jacobian(d, sample, i=6, j=6)[0][0]
...
domainDataset = DomainDatasetRandom([0.0]*num_inputs,[1.0]*num_inputs, 100000, period = 3)
