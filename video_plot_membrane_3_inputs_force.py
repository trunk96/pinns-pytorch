...
exact_solution = "C:\\Users\\desan\\Documents\\Wolfram Mathematica\\file_membrane.csv"

def exact():
    sol = []
    with open(exact_solution, "r") as f:
        for line in f:
            s = line.split(",")[3].strip()
...
def compose_input(x, y, t):
    X = (x-x_min)/delta_x
    Y = (y-y_min)/delta_y
    tau = np.ones_like(x)*(t/t_f)
    x_in = np.vstack((np.ravel(X), np.ravel(Y), np.ravel(tau))).T
