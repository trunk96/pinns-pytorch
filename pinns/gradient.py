...
def jacobian(output, input, i=None, j=None):
    ...
    if i == None:
        ...
        for k in range(output.shape[-1]):
            ...
            d_k = torch.autograd.grad(output, input, grad_outputs=g, create_graph=True)[0]
            grads.append(d_k)
        ...
        g = torch.zeros_like(output)
        g[..., i] = 1
        d = torch.autograd.grad(output, input, grad_outputs=g, create_graph=True)[0]
        if j == None:
            return d
        else:
            return d[:, j]
